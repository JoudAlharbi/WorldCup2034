import { PLAYER_POOL, QUESTIONS_PER_ROUND, REVEAL_TYPES, localRevealImage } from "./playerPool";
...
function pickDistractors(player, pool, count = 3) {
  ...
  const fallback = pool.filter((entry) => entry.id !== player.id);
  const source = sameDifficulty.length >= count ? sameDifficulty : fallback;
...
export function buildQuestionSet(difficulty, readyIds, count = QUESTIONS_PER_ROUND) {
  ...
  return selectedPlayers.map((player) => {
    const revealType = pickRevealType();
    ...
    return {
      ...
      options,
      revealType,
      image: localRevealImage(player.id, revealType),
      hints: {
